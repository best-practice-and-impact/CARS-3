...
tables <- carsurvey3::summarise_all(data, all_tables = TRUE)
...
R, SQL and Python were the three most available programming languages to government analysts. All three are open source languages, the use of which is a key component of the [RAP methodology](https://analysisfunction.civilservice.gov.uk/support/reproducible-analytical-pipelines/). Such tools help analysts make their work more transparent and accessible to users, as cost is not a barrier to learning these languages. 
...
plot <- carsurvey2::plot_likert(tables$capability_change_by_freq, 3, "Capability change", "", 912, font_size = 14, width = 600, height = 800)

tables$capability_change_by_freq[2:6] <- round(tables$capability_change_by_freq[2:6], digits = 1)
...
tables$basic_score_by_implementation[2:8] <- round(tables$basic_score_by_implementation[2:8], digits = 1)
...
tables$basic_score_by_understanding[2:8] <- round(tables$basic_score_by_understanding[2:8], digits = 1) 
