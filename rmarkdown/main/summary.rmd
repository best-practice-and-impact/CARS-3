...
## Most respondents use code at least regularly for various aspects of their work
...
## Each data operation is done some or all the time by code

All the data operations have a majority that is done by some or all by code. There is no measure of how much is done by code which limits this question. This could mean a small amount is done by code and this response would still be selected. 
...
## Open source tools are the most accessible

The three most common coding tools are all open source; R, SQL and python respectively. There is still a high frequency of access to propriety software’s but also more uncertainty around them.

Again, the main advertising of the survey was through the RAP champions who will more likely use open source code.
...
## Open source tools are also the most well known

The three highest access coding tools are all open source; R, SQL and Python. Many propriety software are still well known.  

The questions asked if respondents know how to program with the tool to a level suitable for work.
...
## Many respondents have experience with tools but no access to them

Many of the highest frequencies are propriety tools but there are also open source tools which respondents only have knowledge of. 
